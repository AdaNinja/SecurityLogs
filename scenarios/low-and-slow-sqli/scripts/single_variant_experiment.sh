...
show_usage() {
    ...
    echo "  stealthy, moderate, aggressive, research, production"
    echo ""
    echo "Examples:"
...
collect_data() {
    ...
    if [ "$attacker_logs" -gt 5 ]; then
        docker logs securitylogs-attacker > "$experiment_dir/securitylogs-attacker_logs.txt" 2>&1
        log "✓ Copied attacker logs ($attacker_logs lines)"
    else
        log "⚠️ Attacker logs are empty, skipping"
    fi
    ...
    if [ "$webapp_logs" -gt 10 ]; then
        ...
        log "⚠️ Webapp logs are minimal, skipping"
    fi
...
main() {
    ...
    # Validate variant
    valid_variants=("stealthy" "moderate" "aggressive" "research" "production")
    valid_variant=false
